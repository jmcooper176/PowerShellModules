...
function Get-InUsePackages {
    ...
    $selectedPackageVersions = New-Object -TypeName 'System.Collections.Generic.Dictionary[string,Octopus.Client.Model.PackageResource]'
        ...
        foreach ($action in $step.Actions) {
            if ((!$action.IsDisabled) -and $packageVersions[$action.Name]) {
                foreach ($package in $action.Packages) {
                    $selectedPackageVersion = New-Object -TypeName 'Octopus.Client.Model.PackageResource'
                    ...
                    $normalizedVersion = (New-Object -TypeName 'Octopus.Client.Model.SemanticVersion' -ArgumentList $packageVersions[$action.Name]).ToNormalizedString()
...
function Test-FeedId {
    ...
    if ($script:IncludedFeedIds.Contains($FeedId)) {
        ...
        if ($FeedId -like 'feeds-builtin*') {
            Write-Verbose -Message "Skipping feed '$FeedId'. This script only handles external feeds."
        } elseif (!$script:AllFeeds.ContainsKey($FeedId)) {
            Write-Verbose -Message "Skipping feed '$FeedId'. Its details were not found even though it is associated with one or more packages from one or more releases."
        } elseif (
            ...
            Write-Warning -Message "Skipping feed '$FeedId' ($($script:AllFeeds[$FeedId].FeedUri)). Its Username (API key) is not defined."
        } else {
            $result = $true
        }
        if ($result) {
...
function Test-PackagesWentMissingAfterPreviousRunStarted {
    ...
    if (Test-Path -LiteralPath $missingPackagesFile -PathType Leaf) {
        Write-Verbose -Message "Loading list of packages Octopus reported missing from applicable feeds when the previous run started"
        $packagesMissingWhenPreviousRunStarted = Import-Clixml $missingPackagesFile
        Write-Verbose -Message "$($packagesMissingWhenPreviousRunStarted.PackagesMissing.Count) packages were missing when the previous run started; $($packagesMissingWhenPreviousRunStarted.IncludedFeedIds.Count) applicable feeds were checked."
    } else {
        $packagesMissingWhenPreviousRunStarted = @{ IncludedFeedIds = @() ; PackagesMissing = New-Object -TypeName 'System.Collections.Generic.List[Octopus.Client.Model.PackageResource]' }
        Write-Verbose -Message "Found no preexisting file at $missingPackagesFile"
    }
    $regrets = @()
    ...
    if ($regrets.Count -gt 0) {
        $message = "The following package(s) that should have been retained went missing during or after the previous run of this script: $([string]::Join('; ', ($regrets |ForEach-Object -Process{ $_.FeedId + ' ' + $_.PackageId + ' ' + $_.Version })))"
    ...
    Write-Verbose -Message "Saving list of packages missing from applicable feeds"
...
function Get-PackagesMissingFromFeeds {
    ...
    foreach ($package in $packagesOnAffectedFeeds) {
        $packagesProgress++
        Write-Verbose -Message "Requesting notes for package $packagesProgress of $($packagesOnAffectedFeeds.Count)"
        $uri = [string]::Join('/', @(
            $OctopusUri.TrimEnd('/'),
            $SpaceId,
            'packages',
...
function Get-PackageId {
    ...
    if ([Octostache.VariableDictionary]::CanEvaluationBeSkippedForExpression($PossibleOctostacheExpression)) {
        ...
        $ProgressPreference = $progPref
        $snapshottedVariables = New-Object -TypeName 'Octostache.VariableDictionary'
...
function Remove-Package {
    ...
    if ($PSCmdlet.ShouldProcess($Feed.FeedUri, $operation)) {
        Write-Information -MessageData "$operation from feed '$($Feed.FeedUri)'"
...
function Get-PackageIdAndVersion {
    ...
    # - NuGet CLI (list command from nuget or NuGet.Commands.ListCommandRunner from
    ...
    # - Package PowerShell module (Find-Package): not available on all environments; requires
    #   PowerShell 5.0
    ...
    $packages = New-Object -TypeName 'System.Collections.Generic.HashSet[string]'
    $resultPage = 1
    do {
        $progPref = $ProgressPreference
        $ProgressPreference = 'SilentlyContinue'
...
function PurgeNugetFeeds() {
    ...
    Write-Information -MessageData 'Querying Octopus for releases'
    ...
    if ($UseCachedListOfPackagesInUse.IsPresent) {
        Write-Warning -Message "Using cached $inUsePackagesDescription"
        $inUsePackages = Import-Clixml $inUsePackagesCacheFile
    } else {
        Write-Information -MessageData "Compiling $inUsePackagesDescription"
        $inUsePackages = New-Object -TypeName 'System.Collections.Generic.Dictionary[string,Octopus.Client.Model.PackageResource]'
        ...
        foreach ($release in $releases) {
            ...
            Write-Verbose -Message "Requesting deployment process snapshot for release $releasesProgress of $releasesCount"
        ...
        Write-Verbose -Message "Saving $inUsePackagesDescription"
        $inUsePackages | Export-Clixml $inUsePackagesCacheFile -Force -WhatIf:$false -Confirm:$false
    }

    Write-Information -MessageData 'Querying Octopus for feeds'
    $octopusRepository.Feeds.FindAll() `
        | Where-Object -FilterScript { $_.FeedType -eq [Octopus.Client.Model.FeedType]::NuGet } `
        |ForEach-Object -Process{ $script:AllFeeds[$_.Id] = $_ }

    $inUsePackagesAsArray = New-Object -TypeName 'Octopus.Client.Model.PackageResource[]' -ArgumentList $inUsePackages.Values.Count
    $i=0; $inUsePackages.Values |ForEach-Object -Process{ $inUsePackagesAsArray[$i] = [Octopus.Client.Model.PackageResource]$_; $i++ }
    $allFeedsAsArray = New-Object -TypeName 'Octopus.Client.Model.NugetFeedResource[]' -ArgumentList $script:AllFeeds.Values.Count
    $i=0; $script:AllFeeds.Values |ForEach-Object -Process{ $allFeedsAsArray[$i] = [Octopus.Client.Model.NugetFeedResource]$_; $i++ }
    Test-PackagesWentMissingAfterPreviousRunStarted -Packages $inUsePackagesAsArray -Feeds $allFeedsAsArray

    $inUsePackageNamesWithHighestVersion = New-Object -TypeName 'System.Collections.Generic.Dictionary[string,Octopus.Client.Model.SemanticVersion]'
    foreach ($inUsePackage in $inUsePackages.Values) {
        if ((!$inUsePackageNamesWithHighestVersion[$inUsePackage.PackageId]) `
            -or ((New-Object -TypeName 'Octopus.Client.Model.SemanticVersion' -ArgumentList $inUsePackage.Version) `
                -gt $inUsePackageNamesWithHighestVersion[$inUsePackage.PackageId])
            ) {
            $inUsePackageNamesWithHighestVersion[$inUsePackage.PackageId] = New-Object -TypeName 'Octopus.Client.Model.SemanticVersion' -ArgumentList  $inUsePackage.Version
        }
    }
    $inUseFeedIds = @($inUsePackages.Values | Select-Object -Property FeedId -Unique |ForEach-Object -Process{ $_.FeedId })
    Write-Information -MessageData 'Evaluating and processing feeds'
    $script:AllFeeds.Keys | Where-Object -FilterScript { $inUseFeedIds -notcontains $_ } |ForEach-Object -Process{
        Write-Verbose -Message "Skipping feed '$_' ($($script:AllFeeds[$_].FeedUri)). It is not associated with any packages from any releases."
    }
    foreach ($inUseFeedId in $inUseFeedIds) {
        if (Test-FeedId -FeedId $inUseFeedId) {
            Write-Information -MessageData "Processing feed '$inUseFeedId' ($($script:AllFeeds[$inUseFeedId].FeedUri))."
            ...
            foreach ($packageOnFeed in $packagesOnFeed) {
                ...
                if ($inUsePackageNamesWithHighestVersion.ContainsKey($packageName)) {
                    ...
                    if ($inUsePackages.ContainsKey($hash)) {
                        Write-Verbose -Message "$preserveMessage. It is associated with an Octopus release."
                    } elseif ($packageVersion -gt $inUsePackageNamesWithHighestVersion[$packageName]) { # uses semantic comparison operator override
                        Write-Verbose -Message "$preserveMessage. Its version ($($packageVersion.ToNormalizedString())) is higher than the highest version of '$packageName' associated with an Octopus release ($($inUsePackageNamesWithHighestVersion[$packageName])), so it may have been published for a future release."
                    } else {
                        Remove-Package -Feed $script:AllFeeds[$inUseFeedId] -PackageName $packageName -PackageVersion $packageVersion
                    }
                } else {
                    Write-Verbose -Message "$preserveMessage. No package named '$packageName' is associated with an Octopus release, so it may have been published for a future release."
                }
            }
        }
    }
...
AcquireAssemblies
LoadAssemblies
$script:AllFeeds = New-Object -TypeName 'System.Collections.Generic.Dictionary[string,Octopus.Client.Model.NuGetFeedResource]'
$script:ExcludedFeedIds = @()
$script:IncludedFeedIds = @()
if (!(Test-Path -LiteralPath $PathToStoreDataAcrossRuns -PathType Container)) {
